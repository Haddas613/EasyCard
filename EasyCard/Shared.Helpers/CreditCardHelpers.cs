...
namespace Shared.Helpers
{
    public static class CreditCardHelpers
    {
        ...
        public static string GetCardDigits(string cardNumber)
        {
            ...
            if (cardNumber.Length < 6) cardNumber = cardNumber.PadRight(6, '0');
        ...
        public static string FixdLastFourDigits(string last4digits)
        {
            ...
            if (last4digits.Length < 4) last4digits = last4digits.PadRight(4, '0');
            return last4digits?.Substring(last4digits.Length - 4, 4);
        }

        public static DateTime? ParseCardExpiration(string expirationStr)
        {
            if (string.IsNullOrWhiteSpace(expirationStr)) return null;

            var parts = expirationStr.Split("/");

            if (parts.Length != 2) return null;

            Int32.TryParse(parts[0], out var month);

            if (month < 1 || month > 12) return null;

            Int32.TryParse(parts[1], out var year);

            if (year < 18 || year > 38) return null;
