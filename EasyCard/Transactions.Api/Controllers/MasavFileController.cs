...
namespace Transactions.Api.Controllers
{
    ...
    public class MasavFileController : ApiControllerBase
    {
        ...
        [HttpGet]
        public async Task<ActionResult<SummariesResponse<MasavFileSummary>>> GetMasavFiles(MasavFileFilter filter)
        {
        ...
        [HttpGet("rows")]
        public async Task<ActionResult<SummariesResponse<MasavFileRowSummary>>> GetMasavFileRows(MasavFileRowFilter filter)
        {
            var query = masavFileService.GetMasavFileRows().Filter(filter);
            var numberOfRecordsFuture = query.DeferredCount().FutureValue();

            EnsureExists(await masavFileService.GetMasavFiles().FirstOrDefaultAsync(f => f.MasavFileID == filter.MasavFileID));
