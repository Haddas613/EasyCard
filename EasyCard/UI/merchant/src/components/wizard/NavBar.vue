<template>
  <div>
    <v-app-bar class="ec-toolbar" absolute hide-on-scroll flat color="white">
      ...
      <v-col cols="2" class="d-flex px-0 justify-end" v-if="skippable" @click="onClickSkip()">
        <!-- <v-btn color="primary" icon>
          <v-icon size="2rem">mdi-debug-step-over</v-icon>
        </v-btn> -->
        <v-btn color="primary" class="text-none">{{$t('Skip')}}</v-btn>
      </v-col>
      <v-col cols="2" class="d-flex px-0 justify-end" v-if="closeable" @click="onClickClose()">
        <v-btn color="primary" icon>
          <v-icon icon size="2rem" color="primary">mdi-close</v-icon>
        </v-btn>
      </v-col>
      <v-col cols="2" class="d-flex px-0 justify-end" v-if="!skippable && !closeable">
        <v-spacer></v-spacer>
      </v-col>
    </v-app-bar>
