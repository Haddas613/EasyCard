<template>
  <v-flex fill-height>
    <ec-dialog :dialog.sync="showDetailsDialog">
      <template v-slot:title>{{$t('Invoice')}}</template>
      <template>
    ...
    <v-simple-table>
      <template v-slot:default>
        ...
        <tbody>
          <tr v-for="item in items" :key="item.invoiceHistoryID">
...
export default {
  ...
  async mounted(){
      let history = await this.$api.invoicing.getHistory(this.invoiceId);
      if (history && history.numberOfRecords > 0){
          this.items = history.data;
      }
  },
  methods: {
    showDialog(invoiceHistory) {
      this.selectedHistory = invoiceHistory;
      this.showDetailsDialog = true;
    },
    async loadRequestLog(item){
      this.requestLog = await this.$api.invoicesSystem.getCorrelationLog(this.$formatDate(item.operationDate), item.correlationId);
